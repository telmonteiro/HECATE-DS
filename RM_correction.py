...
class RM_correction:
    """
    Extract the RV component due to the star's motion around the barycentre, excluding the component due to the Rossiter-McLaughlin (RM) effect.
    ...
    def __init__(self, planet_params:dict, time:np.array, CCFs:np.array, model:str, plot_fits:bool, plot_rm:bool, save=None):
        ...
        RM_correction.CCFs_RM_corr = CCFs_corr
        RM_correction.x0_corr = x0_corr

        if plot_rm:
            self._plot_rm(phases, tr_dur, tr_ingress_egress, in_indices, out_indices, x0, poly_coefs, x0_corr, save)


    def _plot_rm(self, phases, tr_dur, tr_ingress_egress, in_indices, out_indices, x0, poly_coefs, x0_corr, save=None):
